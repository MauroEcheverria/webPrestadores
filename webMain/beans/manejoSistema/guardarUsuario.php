<?php
	...
	try {
	    ...
	    $query->bindValue(':usr_id_rol', cleanData("noLimite",0,"noMayuscula",$_POST["newRol"])); 
	    $query->bindValue(':usr_contrasenia', $validacionUsuario->setPassword($usr_cedula)); 
	    $query->bindValue(':usr_id_empresa', 1); 
	    $query->bindValue(':usr_fecha_cambio_contrasenia', $fechaActual_4);
	    $query->bindValue(':usr_contador_error_contrasenia', 0);
	    $query->bindValue(':usr_usuario_creacion', cleanData("siLimite",13,"noMayuscula",$dataSesion["cod_system_user"])); 
	    $query->bindValue(':usr_ip_creacion', getRealIP());
	    $query->bindValue(':usr_nacimiento', cleanData("noLimite",0,"noMayuscula",$_POST["newNacimiento"]));
	    $query->bindValue(':usr_sexo', cleanData("siLimite",1,"noMayuscula",$_POST["usr_sexo"])); 
	    $query->bindValue(':usr_telefono', cleanData("siLimite",10,"noMayuscula",$_POST["usr_telefono"]));
	    ...
	    $query_2->bindValue(':pct_cedula',$usr_cedula);
	    $query_2->bindValue(':pct_nom_completos', $usr_nom_completos); 
	    $query_2->bindValue(':pct_fecha_nacimiento', cleanData("noLimite",0,"noMayuscula",$_POST["newNacimiento"]));
	    $query_2->bindValue(':pct_sexo', cleanData("siLimite",1,"noMayuscula",$_POST["usr_sexo"])); 
	    $query_2->bindValue(':usr_usuario_creacion', cleanData("siLimite",13,"noMayuscula",$dataSesion["cod_system_user"]));
	    $query_2->bindValue(':usr_ip_creacion', getRealIP());
	    $query_2->execute();
