<?php
  ...
  try {
    ...
    $query_1->bindValue(':usr_cod_usuario', cleanData("siLimite",13,"noMayuscula",$dataSesion["cod_system_user"]));
    $query_1->execute();
    ...
    if($validacionUsuario->verifyPassword(cleanData("noLimite",0,"noMayuscula",$_POST["valPacienteAnt"]),$row_1["usr_contrasenia"])) { 
      ...
      $query_2->bindValue(':cts_cod_usuario', cleanData("siLimite",13,"noMayuscula",$dataSesion["cod_system_user"]));
      $query_2->execute();
      ...
      if ($countSi >= 1) {
        ...
        $query_3->bindValue(':usr_cod_usuario', cleanData("siLimite",13,"noMayuscula",$dataSesion["cod_system_user"]));
        ...
        $query_5->bindValue(':cts_cod_usuario', cleanData("siLimite",13,"noMayuscula",$dataSesion["cod_system_user"])); 
        $query_5->execute();
        ...
        $query_4->bindValue(':cts_cod_usuario', cleanData("siLimite",13,"noMayuscula",$dataSesion["cod_system_user"]));
        $query_4->execute();
        ...
        if($query_3 && $query_4 && $query_5) {
          ...
          $data_result["dataModal_1"] = '<img src="../../../dist/img/dct_alert.png" width="30px" heigth="20px">';
          $data_result["dataModal_2"] = 'Información';
          $data_result["dataModal_3"] = 'Se ha detectado un error en la acción requerida, favor escribenos a info@meditategy.com';
          $data_result["dataModal_4"] = '<button type="button" class="btn btn-default btn-estandar-dreconstec" data-dismiss="modal">Cerrar</button>';
          echo json_encode($data_result);
        }
      }
