    ...
    if($validacionUsuario->verifyPassword(cleanData("noLimite",0,"noMayuscula",$_POST["valPacienteAnt"]),$row_1["usr_contrasenia"])) { 
      ...
      if ($countSi >= 1) {
        ...
        if($query_3 && $query_4 && $query_5) {
          $pdo->commit();
          $validacionUsuario->newSesion(cleanData("siLimite",13,"noMayuscula",$_POST["cod_system_user"]));
          ...
          $data_result["dataModal_4"] = '<button type="button" class="btn btn-success" onClick="location.href = '."'".'../principal'."'".'">Cerrar</button>';
          echo json_encode($data_result);
