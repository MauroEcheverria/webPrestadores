...
class enviarXML {
    ...
    public function envioXML($id, $comprobante) {

        include ('tipoRequerimiento.php');
...
<factura id="comprobante" version="1.0.0">
    ...
    <infoAdicional>
        ...
        <campoAdicional nombre="Email">' . $email . '</campoAdicional>
            ...
            $nombre = "comprobantes_electronicos/" . $clave_acceso . ".xml";
            ...
            $ruta_factura = HOST . '/comprobantes_electronicos/' . $clave_acceso . '.xml';
            $ruta_certificado = CERTIFICATE;
            $contraseña = PASS;
            $ruta_respuesta = 'http://localhost/GIT/webFactElectronica/index.php';


            $ejecutar = new ejecutar();
            $domain_dir = $_SERVER['SERVER_NAME'];
            
            $ejecutar->firmarFactura($ruta_factura, $ruta_certificado, $contraseña, $ruta_respuesta, $id);
            return $clave_acceso;
        }
        if ($comprobante == 6) {
            ...
            $sql = "select * from datos_cabecera_electronica  inner join datos_guia_electronica on datos_cabecera_electronica.orden_no = datos_guia_electronica.orden_no where datos_cabecera_electronica.id_comprobante=" . $id;
            ...
            $campo['tipo_comporbante'] = 6;
                ...
                $clave = "" . date('dmY', strtotime($campo['fecha'])) . "" . str_pad($campo['tipo_comporbante'], '2', '0', STR_PAD_LEFT) . "" . $campo['ruc_empresa'] . "" . $campo['ambiente'] . "" . $campo['establecimiento'] . "" . $campo['punto_emi'] . "" . str_pad($id, '9', '0', STR_PAD_LEFT) . "" . str_pad($campo['id'], '8', '0', STR_PAD_LEFT) . "" . $id_tipo_emision . "";
                $digito_verificador_clave = $this->validar_clave($clave);
...
<guiaRemision id="comprobante" version="1.0.0">
    ...
    <infoAdicional>
        ...
        if ($comprobante == 7) {
            ...
            $sql = "select * from datos_cabecera_electronica inner join datos_retencion_electronica on datos_cabecera_electronica.orden_no = datos_retencion_electronica.orden_no where datos_cabecera_electronica.id_comprobante='$id'";
...
<comprobanteRetencion id="comprobante" version="1.0.0">
    ...
    <infoAdicional>
        ...
        if ($comprobante == 4) {
            ...
            $db = db_open();
            $sql = "select * from datos_cabecera_electronica inner join detalle_nota_electronica on datos_cabecera_electronica.nota_no = detalle_nota_electronica.nota_no where datos_cabecera_electronica.id_comprobante=" . $id;
            ...
            $sql = "select * from datos_cabecera_electronica inner join datos_nota_credito where datos_cabecera_electronica.id_comprobante='$id' ";
