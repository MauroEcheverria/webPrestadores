...
function update_review_settings($high, $medium, $low)
{
        ...
        // Update the high risk level
        $stmt = $db->prepare("UPDATE review_levels SET value=:value WHERE name='Alto'");
        $stmt->bindParam(":value", $high, PDO::PARAM_INT);
        $stmt->execute();

        // Update the medium risk level
        $stmt = $db->prepare("UPDATE review_levels SET value=:value WHERE name='Medio'");
        $stmt->bindParam(":value", $medium, PDO::PARAM_INT);
        $stmt->execute();

        // Update the low risk level
        $stmt = $db->prepare("UPDATE review_levels SET value=:value WHERE name='Bajo'");
        $stmt->bindParam(":value", $low, PDO::PARAM_INT);
        $stmt->execute();
...
{
	echo "<select id=\"" . htmlentities($name, ENT_QUOTES) . "\" name=\"" . htmlentities($name, ENT_QUOTES) . "\" class=\"form-field\" style=\"width:120px;\" onClick=\"javascript:showHelp('" . htmlentities($name, ENT_QUOTES) . "Help');updateScore();\">\n";
...
{
        echo "<select id=\"" . htmlentities($name, ENT_QUOTES) . "\" name=\"" . htmlentities($name, ENT_QUOTES) . "\" class=\"form-field\" style=\"width:50px;\" onClick=\"javascript:showHelp('" . htmlentities($name, ENT_QUOTES) . "Help');updateScore();\">\n";
...
function create_dropdown($name, $selected = NULL, $rename = NULL, $blank = true)
{
	...
	*/
	if ($rename != NULL)
	{
		echo "<select id=\"" . htmlentities($rename, ENT_QUOTES) . "\" name=\"" . htmlentities($rename, ENT_QUOTES) . "\" class=\"form-field\" style=\"width:120px;\">\n";
	}
	else echo "<select id=\"" . htmlentities($name, ENT_QUOTES) . "\" name=\"" . htmlentities($name, ENT_QUOTES) . "\" class=\"form-field\" style=\"width:120px;\">\n";
	...
	if ($name == "user" || $name == "category" || $name == "team" || $name == "technology" || $name == "location" || $name == "regulation")
	{
...
function create_multiple_dropdown($name, $selected = NULL, $rename = NULL)
{
	...
	echo "    <option value=\"none\">NINGUNO</option>\n";
...
function create_risk_table()
{
	...
	for ($i=4; $i>=0; $i--)
	{
		...
		if ($i == 4)
		{
			echo "<td rowspan=\"5\"><div class=\"text-rotation\"><b>Consecuencia</b></div></td>\n";
		}
	...
	echo "<td>&nbsp;</td>\n";
        echo "<td>&nbsp;</td>\n";
        echo "<td>&nbsp;</td>\n";
	echo "<td colspan=\"5\" align=\"center\"><b>Probabilidad</b></td>\n";
	echo "</tr>\n";
	echo "</table>\n";
}
 ...
 *************************/
function update_user($user_id, $name, $email, $teams, $admin, $review_high, $review_medium, $review_low, $submit_risks, $modify_risks, $plan_mitigations, $close_risks, $multi_factor)
{
        // Open the database connection
        $db = db_open();

        // Update the user
        $stmt = $db->prepare("UPDATE user set `name`=:name, `email`=:email, `teams`=:teams, `admin`=:admin, `review_high`=:review_high, `review_medium`=:review_medium, `review_low`=:review_low, `submit_risks`=:submit_risks, `modify_risks`=:modify_risks, `plan_mitigations`=:plan_mitigations, `close_risks`=:close_risks, `multi_factor`=:multi_factor WHERE `value`=:user_id");
...
function submit_risk_scoring($last_insert_id, $scoring_method, $CLASSIC_likelihood, $CLASSIC_impact, $AccessVector, $AccessComplexity, $Authentication, $ConfImpact, $IntegImpact, $AvailImpact, $Exploitability, $RemediationLevel, $ReportConfidence, $CollateralDamagePotential, $TargetDistribution, $ConfidentialityRequirement, $IntegrityRequirement, $AvailabilityRequirement, $DREADDamage, $DREADReproducibility, $DREADExploitability, $DREADAffectedUsers, $DREADDiscoverability, $OWASPSkill, $OWASPMotive, $OWASPOpportunity, $OWASPSize, $OWASPDiscovery, $OWASPExploit, $OWASPAwareness, $OWASPIntrusionDetection, $OWASPLossOfConfidentiality, $OWASPLossOfIntegrity, $OWASPLossOfAvailability, $OWASPLossOfAccountability, $OWASPFinancialDamage, $OWASPReputationDamage, $OWASPNonCompliance, $OWASPPrivacyViolation, $custom)
{
	...
	// If the scoring method is OWASP (4)
		...
		$OWASP_impact = ($technical_impact + $business_impact)/2;
                ...
                // Calculate the risk via classic method
                $calculated_risk = calculate_risk($OWASP_impact, $OWASP_likelihood);
...
function update_classic_score($id, $CLASSIC_likelihood, $CLASSIC_impact)
{
        ...
        // Audit log
        $risk_id = $id + 1000;
        $message = "Calificacion del Riesgo para el riesgo id \"" . $risk_id . "\" ha sido actualizado por el usuario \"" . $_SESSION['user'] . "\".";
...
function get_risk_table($sort_order=0)
{
	...
	echo "<th align=\"left\" width=\"150px\">Estado</th>\n";
	echo "<th align=\"left\" width=\"300px\">T&iacutetulo</th>\n";
	echo "<th align=\"center\" width=\"100px\">Riesgo</th>\n";
	echo "<th align=\"center\" width=\"150px\">Enviado el</th>\n";
	echo "<th align=\"center\" width=\"100px\">Tratamiento</th>\n";
	echo "<th align=\"center\" width=\"100px\">Revisi&oacute;n del Comit&eacute;</th>\n";
	echo "</tr>\n";
	echo "</thead>\n";
	echo "<tbody>\n";
...
function get_submitted_risks_table($sort_order=11)
{
        ...
        echo "<tr>\n";
        echo "<th align=\"left\" width=\"50px\">ID</th>\n";
        echo "<th align=\"left\" width=\"300px\">T&iacute;tulo</th>\n";
	echo "<th align=\"center\" width=\"150px\">Fecha de Env&iacute;o</th>\n";
        echo "<th align=\"left\" width=\"150px\">Riesgo Calculado</th>\n";
        echo "<th align=\"left\" width=\"150px\">Estado</th>\n";
        echo "<th align=\"center\" width=\"150px\">Equipo</th>\n";
        echo "<th align=\"center\" width=\"150px\">Enviado</th>\n";
        echo "</tr>\n";
        echo "</thead>\n";
        echo "<tbody>\n";
...
function get_mitigations_table($sort_order=13)
{
        ...
        echo "<th align=\"left\" width=\"50px\">ID</th>\n";
        echo "<th align=\"left\" width=\"300px\">T&iacute;tulo</th>\n";
        echo "<th align=\"left\" width=\"150px\">Fecha de Tratamiento</th>\n";
        echo "<th align=\"left\" width=\"150px\">Tipo de Tratamiento</th>\n";
        echo "<th align=\"center\" width=\"150px\">Esfuerzo</th>\n";
        echo "<th align=\"center\" width=\"150px\">Enviado</th>\n";
        echo "</tr>\n";
        echo "</thead>\n";
        echo "<tbody>\n";
...
function get_reviewed_risk_table($sort_order=12)
{
        ...
        echo "<th align=\"left\" width=\"50px\">ID</th>\n";
	echo "<th align=\"left\" width=\"300px\">T&iacute;tulo</th>\n";
        echo "<th align=\"left\" width=\"150px\">Fecha de Revisi&oacute;n</th>\n";
        echo "<th align=\"left\" width=\"150px\">Revisi&oacute;n</th>\n";
        echo "<th align=\"center\" width=\"150px\">Siguiente Paso</th>\n";
	echo "<th align=\"center\" width=\"150px\">Revisado por</th>\n";
        echo "</tr>\n";
        echo "</thead>\n";
        echo "<tbody>\n";
...
function get_project_list()
{
	...
	echo "<input type=\"submit\" name=\"update_order\" value=\"Actualizar\" /><br /><br />\n";
	...
	echo "</ul>\n";
	echo "<br /><input type=\"submit\" name=\"update_order\" value=\"Actualizar\" />\n";
	echo "</form>\n";
...
function get_project_tabs()
{
	...
	echo "</div>\n";
	echo "<br /><input type=\"submit\" name=\"update_projects\" value=\"Grabar riesgos a proyectos\" />\n";
	echo "</form>\n";
}
...
function get_reviews_table($sort_order=3)
{
        ...
        echo "<th align=\"left\" width=\"300px\">T&iacutetulo</th>\n";
        echo "<th align=\"center\" width=\"100px\">Riesgo</th>\n";
        echo "<th align=\"center\" width=\"100px\">D&iacuteas abiertos</th>\n";
        echo "<th align=\"center\" width=\"150px\">Siguiente fecha de revisi&oacuten</th>\n";
        echo "</tr>\n";
        echo "</thead>\n";
        echo "<tbody>\n";
...
/*****************************
 * FUNCTION: GET Rastro de Auditor&iacute;a *
 *****************************/
function get_audit_trail($id = NULL)
{
	...
	// Otherwise get the full Rastro de Auditor&iacute;a
	else
	{
...
function get_reviews($id)
{
		...
		echo "<p>\n";
		echo "<u>".$date."</u><br />\n";
		echo "Revisado por: ".$reviewer."<br />\n";
		echo "Revisi&oacute;n: ".$review_value."<br />\n";
		echo "Siguiente paso: ".$next_step."<br />\n";
...
function get_Noticias()
{
	$Noticias = "<ul>\n";
        ...
        $announcement_file = file('http://updates.simplerisk.org/Noticias.xml');
	...
	$regex_pattern = "/<announcement>(.*)<\/announcement>/";
        ...
        foreach ($announcement_file as $line)
        {
                if (preg_match($regex_pattern, $line, $matches))
                {
                        $Noticias .= "<li>" . htmlentities($matches[1], ENT_QUOTES) . "</li>\n";
                }
        }
